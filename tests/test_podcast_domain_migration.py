...
def test_migration_can_normalize_already_migrated_pair_link():
    base = "https://perandre.no/1mp"
    pair = f"{base}/papers/a-useful-paper-fc90460f/full/"
    record = {
        "title": "A useful paper — Author (2026)",
        "paper_guid": "fc90460f-d7e7-57c8-a135-81312cb5706b",
        "edition": "brief",
        "page_url": f"{base}/papers/a-useful-paper-fc90460f/brief/",
        "paired_url": pair,
        "show_notes": f"Paired edition: {pair}",
    }
    migrated = migrate_record(record, "https://feed.mere.no", base)
    assert migrated["page_url"].endswith("/brief")
    assert migrated["paired_url"].endswith("/full")
    assert migrated["paired_url"] in migrated["show_notes"]
