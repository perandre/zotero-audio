# Legacy pipeline commands and operational reference

This preserves the previous README for direct pipeline use and recovery. The
current product entry point is [`za`](../README.md), with one persistent Mac
worker and the Cloudflare dashboard/MCP service. Do not enable the old five-minute
scheduler alongside the installed `com.pesh.one-more-paper` worker.

The legacy podcast builder retains older artwork/transcript/chapter behavior
and stricter gates described below. New `za` generation uses optional warning-based
QA, shared show artwork, independent editions and one final audio encode.
The old statement that no article text leaves the Mac applies to these direct
local commands only; the new authenticated cloud mirror includes private Markdown.
...
900 is optimal. See [the engine evaluation](tts-evaluation.md) for the
quality rationale.
...
The [Zotero license-status add-on](zotero-license-status.md) maintains an
...
The same add-on exposes a [read-only local feed API](zotero-feed-api.md)
...
setup are in [the podcast automation plan](podcast-automation-plan.md).
...
McKinsey, Deloitte, and BCG. See [the report workflow](industry-reports.md)
